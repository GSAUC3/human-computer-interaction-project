...
while 1:
    ...
    if landmarks:
        for i in Buttons:
            ...
            if x< landmarks['index'][3][1] < x+i.GAP and y<  landmarks['index'][3][2] <y+i.GAP and depth>169:
            ...
            if x1<  landmarks['index'][3][1] <x1+backspace.GAP and  depth>169 and  y1<  landmarks['index'][3][2] <y1+backspace.GAP:

                cv2.rectangle(img,backspace.position,(x1+backspace.GAP,y1+backspace.GAP),(255, 255, 0),cv2.FILLED)
                cv2.putText(img,i.text,(x1+5,y1+40),cv2.FONT_HERSHEY_COMPLEX,1,(255,255,255),1)
                if string!='':
